...
const GalleryView = () => {
  ...
  useEffect(() => {
    ...
    setShowWhiteRabbit(podeMostrarPista('COELHO'));
  }, []);
  ...
  return (
    <div className="p-8 h-full overflow-auto">
      ...
      <div className="grid grid-cols-2 gap-6">
        {/* PISTA 2: Coelho Branco (Aparece após encontrar PISTA 1) */}
        {showWhiteRabbit && (
          <div
            onClick={handleRabbitClick}
            className="neon-border bg-card/30 backdrop-blur-sm p-4 hover:bg-card/50 transition-all cursor-pointer group relative"
          >
            <div className="aspect-video bg-input/50 flex items-center justify-center mb-4 relative overflow-hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-primary/30 to-secondary/30 opacity-0 group-hover:opacity-100 transition-opacity animate-pulse" />
              <div className="absolute top-2 right-2 px-2 py-1 bg-primary/90 text-primary-foreground text-[10px] terminal-text font-bold animate-pulse">
                ANOMALIA
              </div>
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-2xl text-primary terminal-text tracking-widest font-bold neon-glow">
                  SIGA-ME
                </span>
              </div>
            </div>
            <p className="text-xs text-primary terminal-text tracking-wider animate-pulse">
              WHITE_RABBIT.unknown
            </p>
            <p className="text-[10px] text-muted-foreground terminal-text mt-1">
              CAPTURA: 1999.03.31 // 00:08:43
            </p>
          </div>
        )}
